...
class ExecutionReasonerOutput(NamedTuple):
  loop_state: LoopState
  step_scores: Tensor
  trace_repr: Tensor


class ExecutionReasoner(STRIDETransformer):
  def __init__(self, config: STRIDEConfig) -> None:
    cfg = config.execution_reasoner
    vocab = config.vocab
    ...
    self.step_scorer = nn.Sequential(
      ...
      nn.SiLU(),
      nn.Linear(cfg.d_model // 2, 1, bias=True),
      nn.Sigmoid(),
    )
  ...
  def forward(
    ...
    step_scores = self.step_scorer(loop_state.hidden).squeeze(-1)
    trace_repr = self.pool(loop_state)

    return ExecutionReasonerOutput(
...
if __name__ == "__main__":
  ...
  print("── Building ExecutionReasoner ──────────")
  model = ExecutionReasoner(config)
  ...
  assert out.step_scores.shape == (B, S), \
    f"step_scores: expected ({B}, {S}), got {tuple(out.step_scores.shape)}"
  assert out.trace_repr.shape == (B, config.execution_reasoner.d_model), \
    f"trace_repr: expected ({B}, {config.execution_reasoner.d_model}), got {tuple(out.trace_repr.shape)}"
  ...
  print(f"  step_scores range [{out.step_scores.min():.3f}, {out.step_scores.max():.3f}]  ✓")
  ...
  print("\n✓ ExecutionReasoner test complete")